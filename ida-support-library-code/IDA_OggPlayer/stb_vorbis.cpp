...
/// KW:
#include <intrin.h>
#pragma intrinsic(memset, memcpy, strcat, strcmp, strcpy, strlen, abs, fabs, labs, atan, atan2, tan, sqrt, sin, cos, floor, pow)
...
// ===========================================================
// Local private heap
#define HEAP_FLAGS 0 // HEAP_NO_SERIALIZE
...
// KW: Local private heap setup 
PVOID localAlloc(size_t size, int32 line)
{
    PVOID memblock = HeapAlloc(localHeap, HEAP_FLAGS, size);
    trace("LA P: %08X, S: %u, #%d, sn: %u\n", memblock, size, line, allocSerial);    
    if (allocSerial == 379)
    {
        int breakhere = 0;
    }
...
PVOID localRealloc(PVOID memblock, size_t size, int32 line)
{
    memblock = HeapReAlloc(localHeap, HEAP_FLAGS, memblock, size);
    trace("RA P: %08X, S: %u, #%d\n", memblock, size, line);
    return(memblock);
}

#define MALLOC(_size) localAlloc(_size, __LINE__) /*HeapAlloc(localHeap, HEAP_FLAGS, _size)*/
#define REALLOC(_memblock, _size) localRealloc(_memblock, _size, __LINE__) /*HeapReAlloc(localHeap, HEAP_FLAGS, _memblock, _size)*/
#define FREE(_memblock) HeapFree(localHeap, HEAP_FLAGS, _memblock)
#else
#define MALLOC(_size) HeapAlloc(localHeap, HEAP_FLAGS, _size)
#define REALLOC(_memblock, _size) HeapReAlloc(localHeap, HEAP_FLAGS, _memblock, _size)
#define FREE(_memblock) HeapFree(localHeap, HEAP_FLAGS, _memblock)
#endif

// Free the buffer returned by stb_vorbis_decode_memory()
void stb_vorbis_free_buffer(short *buffer)
{
    if (buffer)
        FREE(buffer);
}

// Destroy local private heap
void stb_vorbis_heap_destroy()
{
    //assert(localHeap != NULL);
    if (localHeap)
    {
        #if 0
        // Validate heap
        assert(HeapValidate(localHeap, HEAP_FLAGS, NULL));

        // Check for memory leaks
        trace("\n===================== Heap Walk =====================\n");
        PROCESS_HEAP_ENTRY hnf;
        uint32 accumulator = 0;
        uint32 unfreedBlocks = 0;
        memset(&hnf, 0, sizeof(hnf));
        hnf.cbData = sizeof(hnf);
        while (HeapWalk(localHeap, &hnf) == TRUE)
        {

 TODO: Try this
 if (entry.wFlags & PROCESS_HEAP_ENTRY_BUSY)
            trace("[%u] P: %08X, S: %u\n", unfreedBlocks, hnf.lpData, hnf.cbData /*+ hnf.cbOverhead*/);
            accumulator += (hnf.cbData + hnf.cbOverhead);
            unfreedBlocks++;
        };
        assert(GetLastError() == ERROR_NO_MORE_ITEMS);
        trace("Total %u bytes leak from %u unfreed blocks.\n", accumulator, unfreedBlocks);
        #endif
...
static uint32 __declspec(align(16)) crc_table[256];
...
const static float __declspec(align(16)) inverse_db_table[256] =
{
...
stb_vorbis * stb_vorbis_open_memory(const unsigned char *data, int len, int *error, stb_vorbis_alloc *alloc)
{
    ...
    // KW: Create the local private heap
    assert(localHeap == NULL);
    localHeap = HeapCreate(HEAP_FLAGS, 0, 0);
    assert(localHeap != NULL);
    ULONG uLFHFlag = 2;
    HeapSetInformation(localHeap, HeapCompatibilityInformation, &uLFHFlag, sizeof(ULONG));
...
int stb_vorbis_decode_memory(const uint8 *mem, int len, int *channels, int *sample_rate, short **output)
{
  //allocSerial = 0;
   ...
   if (v == NULL) return -1;   
   limit = (v->channels * 16384); // KW: Tweaking this up from the default 4096 size to lesson the few reallocations
   *channels = v->channels;
   if (sample_rate)
